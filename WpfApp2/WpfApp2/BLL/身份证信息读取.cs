...
namespace WpfApp2.BLL
{
    ...
    public class GetMessage
    {
        ...
        public void GO()
        {
            rtnTemp = UserApi.SDT_StartFindIDCard(EdziPortID, ref pucIIN, EdziIfOpen);
            if (rtnTemp == 159)
            {
                rtnTemp = UserApi.SDT_SelectIDCard(EdziPortID, ref pucSN, EdziIfOpen);
                if (rtnTemp != 144)
                {
                    rtnTemp = UserApi.SDT_ClosePort(EdziPortID);
                }
                else
                {
                    ReadWriteMsg();
        ...
        public void ReadWriteMsg()
        {
            ...
            try
            {
                rtnTemp = UserApi.SDT_ReadBaseMsgToFile(EdziPortID, "wz.txt", ref puiCHMsgLen, "zp.wlt", ref puiPHMsgLen, EdziPortID);
            }
            catch (Exception ex)
            {
                string m = ex.Message;
            ...
            f.Delete();
            if (bUsbPort)
                rtnTemp = UserApi.GetBmp("zp.wlt", 2);
            else
                rtnTemp = UserApi.GetBmp("zp.wlt", 1);
