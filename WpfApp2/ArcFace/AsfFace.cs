...
namespace AsfFace
{
    /// <summary>
    /// 虹软AsfFace2.0调用封装类
    /// 库.NET Framework 2.0版本(加强兼容性)
    /// 作者:西瓜码农 博客:https://www.xgblog.cn
    /// </summary>
    public class AsfFace
    {
        string AppId = "7jupCbXxmFeBMoXLNVNyawVsALuq78ozS7NmxqW3D4n2";
        string SDKKey = "2xLT15W2cQDPWJ2Zb2cCQgzYpTih14FVqjNmDVsm5Viu";
        int nScale = 16;        
        int nMaxFaceNum = 50;
        ...
        public AsfFace()
        {
            //激活程序(其实激活一次即可,之后激活都会返回错误)
            int res = AsfFunctions.ASFActivation(AppId, SDKKey);
        }
        ...
        {
            int res = 0;
            res = AsfFunctions.ASFUninitEngine(hEngine);
            return res;
        }
        ...
        public int GetImageFaceCount(IntPtr hEngine, byte[] imageData, out int faceCount)
        {
            faceCount = 0;
            try
            {
                ...
                ImageData image = ImageDataConverter.ConvertToImageData(bmap);
        ...
        /// <returns></returns>
        public int FaceMatching(IntPtr hEngine, byte[] imageData1, byte[] imageData2, out float similscore)
        {
            similscore = 0;
            try
            {
                ...
                int res = GetFaceFeature(hEngine, imageData1, out faceFeature1);
                if (res != AsfConstants.MOK)
                    return res;

                res = GetFaceFeature(hEngine, imageData2, out faceFeature2);
                if (res != AsfConstants.MOK)
                    return res;
        ...
        /// <returns></returns>
        public int GetFaceInfo(IntPtr hEngine, byte[] imageData, out List<FaceInfoModel> faceInfo)
        {
        ...
        private int GetFaceFeature(IntPtr hEngine, byte[] imageData, out AsfStruct.ASF_FaceFeature faceFeature)
        {
            ...
            if (detectedFaces.faceNum == 1)
            {
                ...
                CopyMemory(faceFeature.feature, featureout.feature, featureout.featureSize);
                return 0;
            }
            else
            {
